...
const util_1 = __webpack_require__(31);
const server_1 = __importDefault(__webpack_require__(34));
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        const config = coc_nvim_1.workspace.getConfiguration('browser');
        const isEnable = config.get('enable', true);
        if (!isEnable)
            return;
        const { subscriptions, storagePath } = context;
        const stat = yield util_1.statAsync(storagePath);
        if (!stat || !stat.isDirectory()) {
            yield util_1.mkdirAsync(storagePath);
        }
        let capacity = config.get('capacity');
        const port = config.get('port', 8888);
        if (capacity <= 0 || capacity >= 10)
            capacity = 5;
        const server = new server_1.default(capacity, port, storagePath);
        yield server.start();
        const minLength = config.get('minLength');
        const maxLength = config.get('maxLength');
        const browserCompletionProvider = new provider_1.BrowserCompletionProvider(server, minLength, maxLength);
        subscriptions.push(server);
        subscriptions.push(coc_nvim_1.commands.registerCommand('browser.clearCandidates', () => __awaiter(this, void 0, void 0, function* () {
            yield browserCompletionProvider.clearCandidates();
        })));
        subscriptions.push(coc_nvim_1.languages.registerCompletionItemProvider('coc-browser', config.get('shortcut', "WEB"), null, browserCompletionProvider, [], [], config.get('priority', 5)));
    });
}
...
class BrowserCompletionProvider {
    constructor(server, minLength, maxLength) {
    ...
    provideCompletionItems(_document, _position, _token, _context) {
        return __awaiter(this, void 0, void 0, function* () {
            const words = yield this.gatherCandidates();
            return words.map(word => ({
                label: word,
                kind: vscode_languageserver_protocol_1.CompletionItemKind.Text,
                insertText: word,
            }));
        });
    }
    gatherCandidates() {
        return __awaiter(this, void 0, void 0, function* () {
            const words = [];
            const files = yield util_1.readdirAsync(this.sourceDir);
            let sourcePath;
            let content;
            for (const file of files) {
                sourcePath = path_1.default.join(this.sourceDir, file);
                content = yield util_1.readFileAsync(sourcePath);
    ...
    clearCandidates() {
        return __awaiter(this, void 0, void 0, function* () {
            const sourceFiles = yield util_1.readdirAsync(this.sourceDir);
            for (const file of sourceFiles) {
                const filepath = path_1.default.join(this.sourceDir, file);
                yield util_1.rmFileAsync(filepath);
            }
        });
    }
}
exports.BrowserCompletionProvider = BrowserCompletionProvider;
...
const util_1 = __importDefault(__webpack_require__(33));
...
function writeFileAsync(fullpath, content) {
    return __awaiter(this, void 0, void 0, function* () {
        yield util_1.default.promisify(fs_1.default.writeFile)(fullpath, content, 'utf8');
    });
}
exports.writeFileAsync = writeFileAsync;
function readFileAsync(fullpath, encoding = 'utf8') {
    return new Promise((resolve, reject) => {
...
Object.defineProperty(exports, "__esModule", { value: true });
const http_1 = __importDefault(__webpack_require__(35));
const path_1 = __importDefault(__webpack_require__(13));
const coc_nvim_1 = __webpack_require__(1);
const util_1 = __webpack_require__(31);
const dispose_1 = __webpack_require__(36);
class Server extends dispose_1.Dispose {
    constructor(capacity, port, sourceDir) {
        super();
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.server.on('request', (request, response) => {
                ...
                request.on('end', () => __awaiter(this, void 0, void 0, function* () {
                    yield this.saveWords(words);
                }));
                request.on('error', e => {
                    coc_nvim_1.workspace.showMessage(`request error from browser: ${e.message}`);
                });
    ...
    saveWords(text) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const sourcePath = path_1.default.join(sourceDir, `${this.counter % this.capacity}.dat`);
            yield util_1.writeFileAsync(sourcePath, text);
            this.counter++;
        });
    }
}
